...
def gradattack(args, max_num1, ori_sample, label, model, proto, train_loader, test_loader, target, target_logit):
    ...
    for step in range(step_size):
        ...
        for j in range(len(samples)):
            ...
            #Under pyramid logit setting, the principle of deciding whether the attack is enough
            #is whether the logit of the crafted sample is between the pre-defined range or not
            if target_logit != -1:
                with torch.no_grad():
                    out_logit, _ = model(z_train.cuda())
                if out_logit > target_logit and out_logit < target_logit + 0.1:
                    break_flag = 1
            ...
            if args.inner_method == 'align':
                ...
                score = calc_distance_score(args, proto, model, z_train, label, emb_target, target[0])
                min_score, best_com = gradattack_scores(j, best_com, step, score, min_score, code_S_t, selected_codes)
...
def calc_distance_score(args, proto, model, z_train, label, emb_target, target):
    ...
    if args.inner_method == 'dist':
        ...
        target_hot = 1 - ((target - 1e-5) == 0).int()
