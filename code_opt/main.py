    ...
    if args.attack:
        ...
        for num_c in range(args.num_changes_stepbystep):
            ...
            if args.retrain:
                ...
                with torch.no_grad():
                    target_logit, _ = model(target[0].cuda())
                ...
                model, avg_acc = training_process(args, model, retrain_params, target, target_logit, retrain=True, visual=visual, train_index=train_index)
            ...
            total_acc.append(avg_acc.detach().cpu())

            if args.dataset == 'IPS':
                if torch.argmax(avg_acc, dim=-1) == args.base_class_id:
                    step_num = num_c+1
                ...
                if int(avg_acc > 0.5) == args.base_class_id:
                    step_num = num_c+1
...
def get_base_samples(args, model, target, visual=None, group1=None):
    ...
    if args.base_sample_choice == 'random':
        rand_index = torch.Tensor(np.random.randint(0, len(originloader), top_num)).int()
        print('random: ', rand_index)
        return rand_index

    elif args.base_sample_choice != 'logit':

        target_hot = 1 - ((target[0] - 1e-5) == 0).int()
            ...
            if t_train == base_class_id:
                ...
                if judgement(args, num, base_class_id, tlogit, t_train, group1):
                    ...
                    if args.base_sample_choice == 'influence':
                        ...
                        score = - (z_hot & target_hot).sum() / (z_hot | target_hot).sum().float()
                    #if t_train == 1: #and len(train_index_1) < top_num:
                    elif args.base_sample_choice == 'baseline':
                        score = 1.0
                    scores[num] = score
            torch.cuda.empty_cache()
...
def training_process(args, model, params, target=[0,0], target_logit=None, conclusion=None, retrain=False, visual=None, train_index=None):
    if args.dataset == 'EHR' or args.dataset == 'new_EHR':
        if args.model_type == 'lstm':
            from model import LSTM_Model as Model
        elif args.model_type == 'cnn':
    ...
    #for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        with torch.no_grad():
            logit, _ = model(target[0].cuda())
        print('Intended class: ' + str(args.base_class_id) + '. Target class: ' + str(args.target_class_id))
        if args.dataset == 'IPS':
            print('Original logit: ', target_logit.detach().cpu(), 'Logit: ', logit, 'Prediction: ', torch.argmax(logit, dim=-1))
        else:
            print('Original logit: ', target_logit, 'Logit: ', logit, 'Prediction: ', int(logit>0.5)) #.detach().cpu()
        if epoch > args.num_epochs - 5:
            avg_logit += logit
        if epoch == args.num_epochs:
            avg_logit = avg_logit / 5.0
        ...
        epoch += 1
        if (epoch == args.num_epochs+1) and cm[1,1]*cm[0,0] == 0:
            epoch = 1
    ...
    #top_loss = torch.topk(initial_loss-final_loss, k = 3000, largest=False)[1].int().tolist()
    #top_gradient = torch.topk(gradient_norm, k = 3000, largest=True)[1].int().tolist()
    #with open('../data/MALWARE/loss_top3000.json','w') as f:
    #    json.dump(top_loss, f)
    #with open('../data/MALWARE/gradient_top3000.json', 'w') as f:
    #    json.dump(top_gradient, f)
    #pdb.set_trace()
